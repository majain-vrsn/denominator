...
final class VerisignMDNSAllProfileResourceRecordSetApi implements AllProfileResourceRecordSetApi {
	...
	public static final String MDNS_RESOURCE_RECORD_LIMIT_ERROR_MSG = "Resource record limit exceeded";

	private final VerisignMDNS api;
	private final String zoneId;
	private final Integer resourceRecordLimit;

	VerisignMDNSAllProfileResourceRecordSetApi(VerisignMDNS api, String zoneId, Integer resourceRecordLimit) {
		this.api = api;
		this.zoneId = zoneId;
	...
	public void put(ResourceRecordSet<?> rrset) {
        ...
        if (oldRecordSet != null) {
        	...
        	if(newRData.isEmpty() && !equal(oldRecordSet.ttl(), Integer.valueOf(ttlToApply))) {
            	...
            	List<Map<String, Object>> oldRData = ImmutableList.copyOf(filter(oldRecordSet.records(), in(rrset.records())));
	...
	private int getTotalRRCount(GetResourceRecordListType rrListType) {
		ResourceRecordByNameAndTypeIterator rrIterator = new ResourceRecordByNameAndTypeIterator(0, 2, api, rrListType);
	...
	static final class Factory implements denominator.AllProfileResourceRecordSetApi.Factory {
		...
		public VerisignMDNSAllProfileResourceRecordSetApi create(String id) {
			return new VerisignMDNSAllProfileResourceRecordSetApi(api, id, provider.getResourceRecordLimit());
		}
	}
