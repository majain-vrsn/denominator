...
class VerisignMDNSZoneApi implements denominator.ZoneApi {
  ...
  public Iterator<Zone> iterator() {
    ...
    final GetZoneListResType response =
        api.getZones(objectFactory.createGetZoneList(getZoneRequestObj(1)));

    final int pages = (response.getTotalCount() / PAGE_SIZE) + 1;

    final ZoneInfoTransformer transformer = new ZoneInfoTransformer();

    final Iterator<Zone> currentIterator =
        Lists.transform(response.getZoneInfo(), transformer).iterator();
    ...
    return new Iterator<Zone>() {
      ...
      public boolean hasNext() {
        while (!current.hasNext() && i <= pages) {
          ...
          GetZoneListResType response =
              api.getZones(objectFactory.createGetZoneList(getZoneRequestObj(++i)));
  ...
  public Iterator<Zone> iterateByName(String name) {
    ...
    GetZoneInfoResTypeV2 zoneInfoRes =
        api.getZone(new ObjectFactory().createGetZoneInfoV2(request));

    ZoneInfoTransformer transformer = new ZoneInfoTransformer();

    ZoneInfo zoneInfo =
        zoneInfoRes.getPrimaryZoneInfo() != null ? zoneInfoRes.getPrimaryZoneInfo() : zoneInfoRes
            .getSecondaryZoneInfo();

    return zoneInfo != null ? Collections.singletonList(transformer.apply(zoneInfo)).iterator()
        : null;
  }
  ...
  public static class ZoneInfoTransformer implements Function<ZoneInfo, Zone> {
    ...
    public Zone apply(ZoneInfo input) {
      ...
      Zone zone =
          Zone.create(input.getDomainName(), input.getDomainName(), 86400,
              "nil@" + input.getDomainName());
