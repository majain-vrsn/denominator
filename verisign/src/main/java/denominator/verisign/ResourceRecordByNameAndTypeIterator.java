...
class ResourceRecordByNameAndTypeIterator implements Iterator<ResourceRecordSet<?>> {
	...
	public ResourceRecordByNameAndTypeIterator(int offset, int limit, VerisignMDNS api, GetResourceRecordListType rrListType) {
		this.offset = offset;
		this.limit = limit;
		this.api = api;
		this.rrListType = rrListType;
	...
	public boolean hasNext() {
		...
		if(peekingIterator == null || (!peekingIterator.hasNext() && !isPagingRequired && offset < totalCount)) {
			initPeekingIterator();
		}
	...
	private void initPeekingIterator() {
		...
		GetResourceRecordListResType response = api.getResourceRecords(new ObjectFactory().createGetResourceRecordList(rrListType));
		...
		if(remainder >= 1 && returnedRRSetSize >=  newOffset + limit) {
			this.peekingIterator = peekingIterator(response.getResourceRecord().subList(newOffset, newOffset + limit).iterator());
		} else if(remainder >= 1 && totalCount >= newOffset) {
			this.peekingIterator = peekingIterator(response.getResourceRecord().subList(newOffset, Math.min(totalCount, newOffset + limit)).iterator());
		} else {
	...
	public static boolean fqdnAndTypeEquals(ResourceRecord actual, ResourceRecord expected) {
        return actual.getOwner().equals(expected.getOwner()) && actual.getType().equals(expected.getType());
    }
